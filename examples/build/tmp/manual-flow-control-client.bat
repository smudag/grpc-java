...
@rem ##########################################################################
@rem
@rem  manual-flow-control-client startup script for Windows
@rem
@rem ##########################################################################
...
@rem Add default JVM options here. You can also use JAVA_OPTS and MANUAL_FLOW_CONTROL_CLIENT_OPTS to pass JVM options to this script.
...
@rem Execute manual-flow-control-client
"%JAVA_EXE%" %DEFAULT_JVM_OPTS% %JAVA_OPTS% %MANUAL_FLOW_CONTROL_CLIENT_OPTS%  -classpath "%CLASSPATH%" io.grpc.examples.manualflowcontrol.ManualFlowControlClient %*
...
:fail
rem Set variable MANUAL_FLOW_CONTROL_CLIENT_EXIT_CONSOLE if you need the _script_ return code instead of
rem the _cmd.exe /c_ return code!
if  not "" == "%MANUAL_FLOW_CONTROL_CLIENT_EXIT_CONSOLE%" exit 1
exit /b 1
